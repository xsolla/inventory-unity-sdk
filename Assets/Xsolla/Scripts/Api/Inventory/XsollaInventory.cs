...
namespace Xsolla.Inventory
{
	...
	public partial class XsollaInventory : MonoSingleton<XsollaInventory>
	{
		...
		string AnalyticUrlAddition => $"?engine=unity&engine_v={Application.unityVersion}&sdk=inventory&sdk_v={Constants.InventorySdkVersion}";
			...
			get
			{
				if (Application.platform != RuntimePlatform.WebGLPlayer)
				{
					return new List<WebRequestHeader>
					{
						...
						new WebRequestHeader() { Name = "X-SDK", Value = "INVENTORY" },
						new WebRequestHeader() { Name = "X-SDK-V", Value = Constants.InventorySdkVersion.ToUpper() },
					};
				}
				else
					return new List<WebRequestHeader>();
